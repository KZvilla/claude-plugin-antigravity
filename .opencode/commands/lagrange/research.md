...
Instructions:
1. Use the `lagrange_agy_research` tool with:
...
4. If the tool errors because the `network` capability is denied, relay that to the user and stop - do not fall back to answering the research question from memory, and do not silently re-run it with `lagrange_agy_run`.
